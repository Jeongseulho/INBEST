		...
		if (tradingDTO.getCreatedDate().isAfter(LocalDateTime.of(tradingDate, startTime))
			...
			if (tradingDTO.getTradingType().equals(TradingType.BUY)) {
				...
				if (tradingDTO.getPrice() <= price) {
					...
					RedisSimulationUserDTO redisSimulationUserDTO = simulationUserHashOperations.get(simulationHashKey,
						String.valueOf(tradingDTO.getUserSeq()));
					redisSimulationUserDTO.setCurrentMoney(
						redisSimulationUserDTO.getCurrentMoney() + tradingDTO.getPrice());
					simulationUserHashOperations.put(simulationHashKey,
						String.valueOf(redisSimulationUserDTO.getUserSeq()), redisSimulationUserDTO);

					//거래로 인한 주식 개수 변경
					String simulationUserHashKey =
						"simulation_" + tradingDTO.getSimulationSeq() + "_" + tradingDTO.getUserSeq();
					...
					RedisStockUserDTO stockUserDTO = stockUserHashOperations.get(simulationUserHashKey,
						simulationUserKey);
